"""Reference APLS implementation, vendored for validation only.

Extracted from CosmiQ/apls 0.1.0 (Adam Van Etten), Apache License 2.0:
https://github.com/CosmiQ/apls

The published package cannot be installed on a current interpreter: it pins
GDAL==2.4.0 and uses Python-2 style implicit relative imports. The scoring math
needs neither -- GDAL is required only by the geojson/GeoTIFF ingestion and
plotting layers, which are not carried over.

**Every function below is byte-for-byte upstream.** Only this docstring and the
import block are ours. That matters: this module's whole job is to be an
independent check on geo_graphs.metrics, and a reference we had edited would not
be independent. Verify with tests/reference/test_vendored_is_faithful.py.

Adaptation lives in the sibling shims rather than here -- apls_utils.py carries
the two GDAL-free helpers this code calls, and apls_plots.py stubs out the
figure writing. Nothing in geo_graphs imports any of it.
"""
